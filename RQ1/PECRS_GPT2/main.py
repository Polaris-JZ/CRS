...
from utils import seed_everything, SequenceCrossEntropyLoss, count_parameters, load_movie_data
...
# model
parser.add_argument("--decoder", type=str, default="gpt2") # todo: place the HuggingFace checkpoint there
...
# add path
parser.add_argument("--movie_ids_path", type=str, default="data/REDIAL/nltk/movies_with_mentions.csv")
parser.add_argument("--token2id_path", type=str, default="data/REDIAL/nltk/token2id.json")
...
train_paths = [
    root+f"data/{args.dataset_name}/nltk/train_data.json", # size: 10006 -> 65670
    root+f"data/{args.dataset_name}/nltk/train_data.json" # size: 801 -> 8152
]
valid_paths = [
    root+f"data/{args.dataset_name}/nltk/valid_data.json", # size: 1000 -> 6567
    root+f"data/{args.dataset_name}/nltk/valid_data.json" # size: 801 -> 8152
]
test_paths = [
    root+f"data/{args.dataset_name}/nltk/test_data.json", # size: 1342 -> 8329
    #root+f"data/{args.dataset_name}/dev_data_processed", # size: 99 -> 977
    root+f"data/{args.dataset_name}/nltk/test_data.json" # size: 99 -> 993
]
...
args.valid_path = valid_paths[index]
args.test_path = test_paths[index]
args.items_db_path = args.movie_ids_path
...
def main(args):
    ...
    # metadata
    # items
    items_db, new_item_db = load_movie_data(args.items_db_path, args)
    ...
    if args.remove_unused_items:
        ...
        if args.dataset_name == "REDIAL":
            ...
            dev_data = torch.load(root+f"data/{args.dataset_name}/nltk/valid_data.json")
            test_data = torch.load(root+f"data/{args.dataset_name}/nltk/test_data.json")
    ...
    args.items_db = new_item_db
    args.ori_items_db = items_db
    ...
    item_ids = list(args.ori_items_db.keys())
    ...
    # print(args.item_ids_to_pseudo_tokens)
    ...
    with open(args.valid_path, 'r', encoding='utf-8') as f:
        valid_data = json.load(f)
    train_dataset = MovieRecDataset("train",train_data, tokenizer, logger, args)
    valid_dataset = MovieRecDataset("valid", valid_data, tokenizer, logger, args)
    train_dataloader = DataLoader(dataset=train_dataset, shuffle=True, batch_size=args.train_bs, collate_fn=data_collator)
    test_dataset = MovieRecDataset("test", test_data, tokenizer, logger, args)
    test_dataloader = DataLoader(dataset=test_dataset, shuffle=False, batch_size=args.eval_bs, collate_fn=data_collator)
    valid_dataloader = DataLoader(dataset=valid_dataset, shuffle=False, batch_size=args.eval_bs, collate_fn=data_collator)
    # model
    ...
    if args.mode == "train":
        ...
        train_dataloader, test_dataloader, valid_dataloader, model, optimizer, scheduler = accelerator.prepare(
            train_dataloader, test_dataloader, valid_dataloader, model, optimizer, scheduler)
        model = model.to(accelerator.device)

        logger.info("Single-GPU training...")
        training_loop(train_dataloader, valid_dataloader, test_dataloader, tokenizer, model, optimizer, scheduler, criterions, logger, accelerator, args)

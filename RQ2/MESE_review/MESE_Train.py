...
def build_movie_id_mapping(train_data, valid_data, test_data):
    """
    构建电影名称到整数ID的映射
    返回:
        movie2id: dict, 电影名称到整数ID的映射
        id2movie: dict, 整数ID到电影名称的映射
    """
    movie_names = set()
    ...
    for dataset in [train_data, valid_data, test_data]:
        for dialogue in dataset:
            for message in dialogue['messages']:
                if 'movie' in message and message['movie']:
                    movie_names.update(message['movie'])
    
    # 建立映射关系
    movie2id = {name: idx for idx, name in enumerate(sorted(movie_names))}
    id2movie = {idx: name for name, idx in movie2id.items()}
    
    return movie2id, id2movie

def get_id2review(train_data, valid_data, test_data):
    import pickle
    train_review_path = "./DATA/dataset/redial/nltk/train_conv_idx_to_review_info.pkl"
    valid_review_path = "./DATA/dataset/redial/nltk/valid_conv_idx_to_review_info.pkl"
    test_review_path = "./DATA/dataset/redial/nltk/test_conv_idx_to_review_info.pkl"

    token2id_path = "./DATA/dataset/redial/nltk/token2id.json"
    entity2id_path = "./DATA/dataset/redial/nltk/entity2id.json"

    train_raw_json = "./DATA/dataset/redial/nltk/train_data.json"
    valid_raw_json = "./DATA/dataset/redial/nltk/valid_data.json"
    test_raw_json = "./DATA/dataset/redial/nltk/test_data.json"
    ...
    for entity_name, review in entity_name_to_review.items():
        item_id = entity_name_to_real_id[entity_name]
        item_id_to_review[item_id] = review

    return item_id_to_review
        

def combine_item_review(id2movie, id2review):
    items_db = {}
    for item_id, item_info in id2movie.items():
        if item_id not in items_db:
            items_db[item_id] = {}
        if item_id in id2review:
            items_db[item_id]['review'] = id2review[item_id]
            items_db[item_id]['title'] = item_info
        else:
            items_db[item_id] = {'review': '', 'title': item_info}
    return items_db
...
# In[5]:
# 构建电影ID映射
# movie2id, id2movie = build_movie_id_mapping(torch.load(train_path), torch.load(valid_path), torch.load(test_path))
...
valid_dataset = MovieRecDataset(torch.load(valid_path), bert_tokenizer, gpt_tokenizer)
...
id2review = get_id2review(torch.load(train_path), torch.load(valid_path), torch.load(test_path))

items_db = combine_item_review(items_db, id2review)
...
class UniversalCRSModel(torch.nn.Module):
    ...
    def get_movie_title(self, m_id):
        title = self.items_db[m_id]["title"]
        title = title.split('[SEP]')[0].strip()
        return title

    def get_movie_review(self, m_id):
        """获取电影评论"""
        movie_info = self.items_db[m_id]
        return movie_info["review"] 
    ...
    #     # cut review's length first
    #     for key in chunk_ids:
    #         if len(items_db_to_use[key]['review']) > 16:
    #             items_db_to_use[key]['review'] = items_db_to_use[key]['review'][:16]
        
    #     chunk_infos = [
    #         f"{items_db_to_use[key]['title']} [SEP] {items_db_to_use[key]['review']}"
    #         for key in chunk_ids
    #     ]
    #     chunk_tokens = self.encoder_tokenizer(chunk_infos, padding=True, truncation=True, max_length=512, return_tensors="pt")
    ...
    def compute_encoded_embeddings_for_items(self, 
        ...
        # 分别编码title和review
        title_infos = [items_db_to_use[key]['title'] for key in chunk_ids]
        review_infos = [' '.join(items_db_to_use[key]['review'][:16]) for key in chunk_ids]
        
        # 编码title
        title_tokens = self.encoder_tokenizer(title_infos, padding=True, truncation=True, 
                                            max_length=512, return_tensors="pt")
        title_input_ids = title_tokens['input_ids'].to(self.device)
        title_attention_mask = title_tokens['attention_mask'].to(self.device)
        title_hiddens = encoder_to_use(input_ids=title_input_ids, 
                                    attention_mask=title_attention_mask).last_hidden_state

        # 编码review 
        review_tokens = self.encoder_tokenizer(review_infos, padding=True, truncation=True,
                                            max_length=512, return_tensors="pt")
        review_input_ids = review_tokens['input_ids'].to(self.device)
        review_attention_mask = review_tokens['attention_mask'].to(self.device)
        review_hiddens = encoder_to_use(input_ids=review_input_ids,
                                    attention_mask=review_attention_mask).last_hidden_state

        # 对title做pooling
        title_mask_expanded = title_attention_mask.unsqueeze(-1).expand(title_hiddens.size())
        title_masked = torch.mul(title_hiddens, title_mask_expanded)
        title_pooled = torch.sum(title_masked, dim=1) / torch.sum(title_attention_mask, dim=1).unsqueeze(-1)

        # 对review做pooling
        review_mask_expanded = review_attention_mask.unsqueeze(-1).expand(review_hiddens.size())
        review_masked = torch.mul(review_hiddens, review_mask_expanded)
        review_pooled = torch.sum(review_masked, dim=1) / torch.sum(review_attention_mask, dim=1).unsqueeze(-1)

        # 使用fusion gate融合
        chunk_pooled = self.fusion_gate(title_pooled, review_pooled)
        
        return chunk_pooled, title_hiddens
...
param_optimizer = list(model.language_model.named_parameters()) +     list(model.recall_encoder.named_parameters()) +     list(model.item_encoder.named_parameters()) +     list(model.recall_lm_query_mapper.named_parameters()) +     list(model.recall_item_wte_mapper.named_parameters()) +     list(model.rerank_item_wte_mapper.named_parameters()) +     list(model.rerank_logits_mapper.named_parameters())
...
#     past_list = []
    ...
    for turn_num in range(len(role_ids)):
        ...
        if recommended_ids != None:
            for r_id in recommended_ids:
                item_ids.append(item_id_2_lm_token_id[r_id])
                title = model.items_db[r_id]['title']
                title = title.split('[SEP]')[0].strip()
...
for ep in range(num_epochs):
    ...
    for batch in pbar:
        ori_sentences, sentences, ic, tc, vc, tgc, rwi, group = validate_language_metrics_batch(batch[0], model, item_id_2_lm_token_id)
        for s in sentences:

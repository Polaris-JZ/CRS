...
class UniversalCRSModel(torch.nn.Module):
    def __init__(self, 
        ...
        # 获取实体嵌入
        entity_emb = self.entity_encoder.get_entity_embeds()
        # 直接创建embedding层，因为entity_emb已经是tensor了
        self.entity_emb = torch.nn.Embedding(self.kg_info['num_entities'], self.language_model.config.n_embd)
        # 直接赋值，不需要torch.from_numpy()
    ...
    def compute_encoded_embeddings_for_items(self, 
        ...
        if entity_list is not None:
            ...
            chunk_pooled = self.fusion_gate(chunk_pooled, batch_entity_emb.unsqueeze(0))
...
#             past_ids = trimmed_ids[:, self.lm_trim_offset + num_items_wtes:self.language_model.config.n_positions]
    ...
    def forward_recall(self, 
        ...
        gt_item_wte, _ = self.compute_encoded_embeddings_for_items(self.recall_encoder, [gt_item_id], self.items_db, None)
        ...
        # # concat past_wtes and entity_emb
        # single_entity_emb = torch.stack([self.entity_emb[int(entity_id)] for entity_id in overall_entity])
        # average_entity_emb = torch.mean(single_entity_emb, dim=0)
        # normalized_entity_emb = F.normalize(average_entity_emb, p=2, dim=-1).unsqueeze(0).unsqueeze(0)
...
param_optimizer = list(model.language_model.named_parameters()) +     list(model.recall_encoder.named_parameters()) +     list(model.item_encoder.named_parameters()) +     list(model.recall_lm_query_mapper.named_parameters()) +     list(model.recall_item_wte_mapper.named_parameters()) +     list(model.rerank_item_wte_mapper.named_parameters()) +     list(model.rerank_logits_mapper.named_parameters()) +     list(model.entity_encoder.named_parameters()) +    list(model.fusion_gate.named_parameters())
...
def past_wtes_constructor(past_list, model):
    ...
    for language_tokens, recommended_ids, overall_entity in past_list:
        ...
        if language_tokens == None and (recommended_ids != None and recommended_ids != []): # rec turn
            ...
            if len(overall_entity) > 0:
                entity_ids = torch.tensor([int(entity_id) for entity_id in overall_entity], device=model.device)
                single_entity_emb = model.entity_emb(entity_ids)
                average_entity_emb = torch.mean(single_entity_emb, dim=0)
                normalized_entity_emb = F.normalize(average_entity_emb, p=2, dim=-1).unsqueeze(0).unsqueeze(0)
...
for ep in range(num_epochs):
    ...
    for batch in pbar:
        total_sentences, sentences, ic, tc, vc, tgc, rwi, group = validate_language_metrics_batch(batch[0], model, item_id_2_lm_token_id)
        for s in sentences:

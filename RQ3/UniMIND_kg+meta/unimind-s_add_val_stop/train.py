...
class DataFrame(Dataset):
    ...
    def __getitem__(self, index):
        return self.source_ids[index][-self.max_len:], self.target_ids[index][-self.max_tgt_len:], self.item_ids[index], self.entity_ids[index], self.item_num
...
def collate_fn(data):
    source_ids, target_ids, item_ids, entity_ids, item_num = zip(*data)
    ...
    entity_ids = [torch.tensor(entity_id).long() for entity_id in entity_ids]
    entity_ids = pad_sequence(entity_ids, batch_first=True, padding_value=0)

    # add entity_attn (padding value is 0)
    entity_attn = torch.zeros_like(entity_ids)
    entity_attn[entity_ids != 0] = 1
    ...
    for e in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {'input_ids': batch['input_ids'].to(args.device),
                        ...
                        'entity_attn':batch['entity_attn'].to(args.device),
                        }
        ...
        # Log metrics
        total_rouge, ndcg10 = evaluate(args, model, tokenizer, 'valid', task, save_output=True)
        ...
        if task == 'item':
            if ndcg10 > best_ndcg:
                ...
                # test
                logging.info("testing in the test set")
                evaluate(args, model, tokenizer, 'test', save_output=True)
            else:
                early_stop_count += 1
                if early_stop_count >= 4:
                    logging.info("Early stopping at epoch %d due to no improvement in validation recall.", e)
                    break
        else:
            if total_rouge[2] > best_f1:
                best_f1 = total_rouge[2]
                early_stop_count = 0
                # Save model checkpoint
                if task is not None:
                    output_dir = os.path.join(args.output_dir, task, 'best_checkpoint')
...
def evaluate(args, model, tokenizer, mode, eval_task=None, save_output=False):

    #results = {}
    eval_dataset = data_reader.load_and_cache_examples(args, tokenizer, mode, evaluate=True)
    ...
    for task in tasks:
        ...
        ndcg10 = 0
        if task == 'item':
            ndcg10 = auto_scores[0]
        logging.info(auto_scores)
        # print(auto_scores)
        all_metrics += auto_scores
    return all_metrics, ndcg10



def pipeline(args, model, tokenizer, mode, save_output=False):
    ...
    for task in ['know', 'item', 'resp']:
        ...
        if args.gt_task == task:
            all_preds[task] = targets
        else:
...
def main():
    ...
    parser.add_argument("--cache_dir", default='/projects/0/prjs1158/KG/model', type=str,
                        help="The cache directory.")
    parser.add_argument("--gt_task", default=None, type=str,
                        help="Pipeline ground-truth task.")
    ## Other parameters
    ...
    parser.add_argument("--max_seq_length", default=511, type=int,
    ...
    # path
    parser.add_argument("--items_db_path", type=str, default="/projects/0/prjs1158/KG/redail/UniMIND_update/data/redail/nltk/movies_with_mentions.csv")
    parser.add_argument("--train_path", type=str, default="/projects/0/prjs1158/KG/redail/UniMIND_update/data/redail/nltk/train_data.json")
    parser.add_argument("--valid_path", type=str, default="/projects/0/prjs1158/KG/redail/UniMIND_update/data/redail/nltk/valid_data.json")
    parser.add_argument("--test_path", type=str, default="/projects/0/prjs1158/KG/redail/UniMIND_update/data/redail/nltk/test_data.json")
    ...
    config = BartConfig.from_pretrained(args.model_name_or_path, cache_dir=args.cache_dir, revision=revision)
    tokenizer = BertTokenizer.from_pretrained(args.model_name_or_path, do_lower_case=args.do_lower_case, cache_dir=args.cache_dir, revision=revision)
    tokenizer.add_special_tokens({'additional_special_tokens':['[goal]','[user]','[system]','[knowledge]','[item]','[profile]','[history]']})

    ft_dataset = data_reader.load_and_cache_examples(args, tokenizer, 'train', evaluate=False)
    ...
    if args.do_pipeline:
        tokenizer = BertTokenizer.from_pretrained(output_dir, do_lower_case=args.do_lower_case)
        model.to(args.device)
        pipeline(args, model, tokenizer, 'test', save_output=True)

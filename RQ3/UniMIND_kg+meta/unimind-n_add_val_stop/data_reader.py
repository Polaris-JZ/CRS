...
def convert_to_features(args, tokenizer, mode):
    ...
    # load entity2id
    entity2id_path = "/projects/prjs1158/KG/redail/MESE_kg/DATA/nltk/entity2id.json"
    entity2id = json.load(open(entity2id_path, 'r', encoding='utf-8'))
    ...
    # 初始化数据字典，移除了'goal'任务
    data_dict = {
        'resp': {'source_ids':[], 'target_ids':[], 'item_ids':[], 'entity_ids':[]}, 
        'item': {'source_ids':[], 'target_ids':[], 'item_ids':[], 'entity_ids':[]}, 
        'know': {'source_ids':[], 'target_ids':[], 'item_ids':[], 'entity_ids':[]}
    }
    ...
    for d in tqdm(data):
        ...
        for utt in conv[1:]:
            ...
            filter_overall_entity = [entity2id[entity] for entity in overall_entity if entity in entity2id]
            ...
            data_dict['resp']['entity_ids'].append(filter_overall_entity)
            ...
            data_dict['know']['entity_ids'].append(filter_overall_entity)
            ...
            if utt['movies']:
                ...
                for i, item_new_id in enumerate(item_new_ids):
                    ...
                    data_dict['item']['entity_ids'].append(filter_overall_entity)
                    rec_index.append(i)
...
def merge_dataset(ft_dataset):
    ...
    for task in ['resp','know','item']:
        ...
        for source_id, target_id, item_id, entity_id in zip(task_dataset['source_ids'], task_dataset['target_ids'], task_dataset['item_ids'], task_dataset['entity_ids']):
            source_ids.append(source_id)
            target_ids.append(target_id)
            item_ids.append(item_id)
            entity_ids.append(entity_id)
    return {'source_ids':source_ids, 'target_ids':target_ids, 'item_ids':item_ids, 'entity_ids':entity_ids, 'item_dict':item_dict}

...
class KGPrompt(nn.Module):
    ...
    def forward(self, entity_ids=None, review=None, token_embeds=None, output_entity=False, use_rec_prefix=False,
                use_conv_prefix=False):
        batch_size, entity_embeds, entity_len, token_len = None, None, None, None
        if review is not None:
            entity_len = 1
            batch_size = review['input_ids'].shape[0]
            ...
            attention_mask = review['attention_mask']
            entity_embeds = self.bert_model(input_ids=input_ids, attention_mask=attention_mask)[0]
            entity_embeds = torch.mean(entity_embeds, dim=1).unsqueeze(1)

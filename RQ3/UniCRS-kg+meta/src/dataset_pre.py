...
class CRSDataset(Dataset):
    def __init__(
        ...
        self.data = []
        self.id2review = pickle.load(open(os.path.join(dataset_dir, 'id2review.pkl'), 'rb'))
        ...
        with open(data_file, 'r', encoding='utf-8') as f:
            ...
            for line in tqdm(lines):
                ...
                for rec in dialog['rec']:
                    review = []
                    for entity_id in dialog['entity'][-self.entity_max_length:]:
                        if entity_id in self.id2review:
                            review.append(self.id2review[entity_id])
                        else:
                            review.append('')

                    review = " ".join(review)
                    # review = [self.id2review[entity_id] for entity_id in dialog['entity'][-self.entity_max_length:]]
...
class CRSDataCollator:
    ...
    def __call__(self, data_batch):
        ...
        for data in data_batch:
            ...
            review_batch.append(data['review'])
        ...
        review_batch = self.bert_tokenizer(review_batch, padding=True, truncation=True, max_length=32, return_tensors='pt')
        for k, v in review_batch.items():
            if not isinstance(v, torch.Tensor):
                review_batch[k] = torch.as_tensor(v, device=self.device)
        input_batch['review'] = review_batch

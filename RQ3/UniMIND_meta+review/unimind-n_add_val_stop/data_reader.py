...
def replace_movie_ids_with_names(utt, items_db, name2review=None):
    ...
    if not name2review:
        ...
        for cnt, mid in enumerate(movie_ids):
            mid = int(mid)
            if cnt < len(movies):
                if movies[cnt] in name2review:
                    review = ' '.join(name2review[movies[cnt]])
                else:
                    review = ''
            else:
                review = ''
            if mid in items_db:
                movie_name = items_db[mid]['movieName']
                movie_meta = items_db[mid]['meta']
                meta_list.append(movie_name + movie_meta + review)
                replaced_text = replaced_text.replace(f'@{mid}', movie_name)
            else:
                meta_list.append('' + review)
    ...
    for d in tqdm(data):
        ...
        for utt in conv[1:]:
            if utt['role'] == 'Seeker':
                ...
                continue
            # 系统回复处理
            system_text, movies_text = replace_movie_ids_with_names(utt, items_db, name2review=name2review)
